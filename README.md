...
先看已经接管的任务流，再看角色分工。NERV 的人物表不是为了做设定，而是为了让你知道每条真实可跑通的作战链该交给谁。

### 指挥层（三柱体制）

| 机体 | 代号 | 当前职责 | 已接管的任务流 | 可调用能力 |
|:-----|:-----|:---------|:---------------|:-----------|
| 🎯 碇源堂 | `nerv-gendo` | 战略顾问 · 需求翻译 · 任务草案 · 路线选择 · 工具发现决策 · 发布授权 | 所有需要先拆解、先补问、先出草案的复杂任务 | `read` `memory_search` `sessions_send` `exec`（仅 `nerv-publisher`） |
| 🗡️ 葛城美里 | `nerv-misato` | 作战指挥官 · 任务建图 · 分派节点 · 巡检状态 · 终态收口 | 直播脚本、直播复盘、社媒文案、商品评价洞察、会议转任务、财讯简报、社媒晨报 | `read` `write` `exec`（工具脚本） `sessions_send` |
| 🔒 SEELE | `nerv-seele` | 安全审计会 · 风控熔断 · 敏感拦截 · 孤岛/漏调度/环路标记 · 全链路审计 | 安全巡检、异常发现、审批前拦截 | `read` `write` `exec`（仅安全脚本） |

### 编排层

| 机体 | 代号 | 当前职责 | 已接管的任务流 | 可调用能力 |
|:-----|:-----|:---------|:---------------|:-----------|
| 🔬 赤木律子 | `nerv-ritsuko` | 代码/适配器编排 · 安装验证 · 质量验收 · 资产落地 | 适配器接入、安装检查、可执行资产上线 | `read` `write` `exec` |
| ⚡ 碇真嗣 | `nerv-shinji` | 数据编排 · 字段结构锚点 · 任务卡整理 | 会议转任务、结构化任务卡、后续数据链编排 | `read` `write` `exec` `memory_search` |
| 🌙 绫波零 | `nerv-rei` | 记忆守护 · 模式提纯 · 经验沉淀 · 损坏隔离 | 直播脚本套路沉淀、复盘模式沉淀、语感/钩子沉淀 | `read` `write` `memory_search` |

### 作战层（一次性电池）

| 机体 | 代号 | 当前职责 | 已接管的任务流 | 可调用能力 |
|:-----|:-----|:---------|:---------------|:-----------|
| 🔥 式波明日香 | `nerv-asuka` | 代码调试 · 故障定位 · 空载验证 | 调试与回归验证链路 | `exec` |
| 🎵 渚薰 | `nerv-kaworu` | 代码审查 · 安全复核 · 兼容性复核 | 适配器审核、安全审查 | `read` `write` `memory_search` |
| 🕷️ 真希波 | `nerv-mari` | 网页抓取 · 图片/文本抽取 · 平台适配 | 网页翻译链、外部内容抓取链 | `exec` |
| 📡 二号机 | `nerv-eva02` | 社媒信号监控 · 趋势预警 | 社媒晨报、信号监测、热点选题 | `read` `write` `memory_search` |
| 🔍 三号机 | `nerv-eva03` | 深度搜索 · 工具发现 · 资料核验 | 工具发现、资料补证、外部能力侦察 | `read` `exec`（`gh` CLI） |
| 🧹 零号机 | `nerv-eva00` | 数据清洗 · 输入归一化 · 字段白名单 · 完整性评分 | 直播脚本前置整理、商品评价洞察前置整理、会议任务结构化、财讯信息整理 | `read` `write` |
| ✍️ 十三号机 | `nerv-eva13` | 文案生成 · 脚本/口播成稿 · 多语言成稿 | 直播脚本、直播复盘总结、社媒多平台文案、财讯简报终稿 | `read` `write` |
| 🛠️ 初号机 | `nerv-eva01` | 部署运维 · 安装校验 · 工具链落地 | 安装脚本、环境检查、部署校验 | `exec` |
| 🎨 量产机 | `nerv-eva-series` | 视觉生成 · 配图 · 图像素材变体 | 配图、图像变体、视觉素材输出 | `read` `write` `image-gen` |

### 神明节点（功能函数，非 Agent）

| 节点 | 实现 | 当前职责 |
|:-----|:-----|:---------|
| 亚当 (Adam) | `adam_notifier.py` 飞书 Webhook | 审批推送、任务完成/失败通知，负责把最终战报送到造物主面前 |
| 莉莉丝 (Lilith) | Cron 脚本 | 每日 03:00 备份 `nerv.db` |
| 朗基努斯之枪 (Spear) | `spear_sync.js` 状态对齐 | 每 5 分钟扫描孤岛、漏调度、环路、异常节点，只负责发现和标记，不自动修复 |

### 已验证任务流能力

先看能跑通什么，再看谁负责。NERV 对外真正要讲清楚的，不只是“找谁”，而是“这条任务流能做什么、怎么触发、会产出什么”。

| 中文触发 | 命中的任务流 | 核心产物 | 主责角色 |
|:--|:--|:--|:--|
| `帮我把这场直播复盘一下` / `整理下次优化点` | 直播复盘链（`live-replay-summary`） | 复盘总结、问题聚类、下一场优化清单 | `Misato` → `Eva-00` → `Eva-13` → `Rei` |
| `把这场直播的脚本做出来` / `给我一套能直接上播的话术` | 直播脚本链（`live-session-script`） | `offer_pack.json`、`script.md`、`selling_points.md`、`cta.md` | `Misato` → `Eva-00` → `Eva-13` → `Rei` |
| `给微博/小红书/抖音各出一版内容` | 多平台内容链（`social-copy-studio`） | 平台分发文案、视频脚本、口播稿 | `Misato` → `Eva-00` → `Eva-13` → `Rei` |
| `把这批商品评价整理成洞察` / `做一版 SKU 卖点简报` | 商品评价洞察链（`product-review-insight`） | 卖点洞察、痛点聚类、SKU 简报 | `Misato` → `Eva-00` → `Eva-13` |
| `把这段会议纪要转成任务清单` / `把周会内容拆成 owner 和 deadline` | 会议转任务链（`meeting-to-task`） | 任务清单、owner、deadline、状态摘要 | `Misato` → `Shinji` → `Eva-00`/`Eva-13` |
| `给我做一版今天的财讯简报` / `看一下这只股票最近有什么值得关注` | 财讯简报链（`finance-brief`） | 财讯简报、风险卡片、观察重点 | `Misato` → `Eva-02` → `Eva-00` → `Eva-13` |
| `我想看今天的社媒热点` / `先给我一版内容选题` | 社媒晨报链（`social-topic-daily`） | 热点简报、选题推荐、标题建议 | `Eva-02` → `Eva-00` → `Eva-13` → `Misato` |

这些任务流不是“会话里说得出来”就算成立，而是已经能在 `nerv.db`、任务会话和模板资产里对得上。用户真正看见的，是成品和通知，不是中间的术语。
...
1. **不是每条任务流都要让所有机体出场**
2. **节点 owner 先看职责，再看能力包**
3. **真相源是 `tasks + dag_nodes`，不是聊天消息本身**

---

## 💡 为什么不一样

核心差异只有一句话：**NERV 不是把更多能力塞进一个更大的脑袋，而是把复杂任务拆成可调度、可审计、可复用的作战链。**

所以它和常见做法的区别也很直接：

- 不是把采集、清洗、成稿、通知塞进一个大脚本
- 不是让多个 Agent 在对话里互相商量到失控
- 而是由 `Gendo` 拆解、`Misato` 建图、机体分节点执行、`Recorder / Spear / Adam / nerv.db` 负责收口

这样做的结果只有三个：

1. 功能扩张不等于上下文膨胀
2. 失败可以局部隔离、局部重试
3. 跑通的链路可以沉成模板，后面直接复用
...
这一层只回答一件事：为什么 NERV 不把模型输出直接当真。代码、部署、外部工具接入，全部先过物理防线，再进主链。
...
> 关键设计决策：日志和状态由物理脚本直接写库，不再让大模型读完输出再“顺手记账”。
...
这一块只解决一个问题：**当前能力不够时，怎么把外部工具接进来，并让它下次能复用。**
...
用户需求 → misato: "没有匹配的 Skill"
          ...
          asuka: 构造虚拟 input.json 进行空载测试 (Dry-Run)
              ↓ 测试通过
          misato: skill_registry 新增一条可复用路由
              ↓
          ✅ 系统获得新的外部能力
```
...
系统能接 GitHub 开源工具，靠的是这套统一契约。无论底层是 Python 还是 Node.js，所有外部工具都要被封成同一种黑盒：
...
- **找 Gendo**：需求还没拆清、你想先看草案、你怀疑当前工具不够
- **找 Misato**：目标已经明确、你要直接执行、你要查进度和终态
- 如果两人不在同一聊天入口，用户本人就是转交层
...
上面的「全员阵列」已经前置列出了当前能完整覆盖的 workflow 能力、中文触发方式和主责角色。这里保留的是安装后的资源位置与输入要求。
...
NERV 现在除了正式的 DAG 工作流，也可以在本机接入两类外部 CLI，作为“前置提炼层”或“代码工兵层”来补强能力：
...
一句话理解：
- **Codex 更像本地代码工兵**，适合动代码、改文件、批量处理工程任务。
...
一句话理解：
- **Gemini 更像本地研究工位**，适合先看懂、先压缩、先提炼，再交给正式工作流。

### 什么时候用它们

- 用户给了很多材料，但还不适合直接建 DAG
- 需要先把资料压成结构化要点
- 需要先做一版补丁草稿，再回到 NERV 正式链路验收
- 正式 workflow 还没长出对应能力，但本地 CLI 可以先补一层
...
MCP 指的是 **Model Context Protocol**，可以理解成“模型调用外部工具的标准接口”。

在 NERV 里，`MCP 工具` 不是一个抽象口号，而是指：
- 通过统一协议把外部能力接进来
- 让 Agent 或 CLI 能安全地调用文件、网页、代码仓库、搜索、知识库等工具
- 把“能调用什么”从 prompt 里拿出来，变成可配置、可审计、可替换的外部能力

更直白一点：
- **MCP 工具 = 让模型可以稳定接外部工具的插槽**
- 扩展 MCP 工具，就是给系统继续加“可调用的外部能力”，而不是继续堆 prompt

...
| 层级 | 角色 | 原则 |
|:-----|:-----|:-----|
| **战略层** | gendo | 不执行，只决策：需求翻译 · 工具发现 · 结果反馈 |
| **战术层** | misato | 不执行具体任务，只编排调度：DAG 路由 · 任务分发 · 巡检 |
| **编排层** | ritsuko · shinji · rei · seele | 局部自治 · 管辖下游 · 聚合评估 |
...
**简单记忆法**：
- **Gendo = 你想做什么**（战略层：聊需求、审方案、确认结果）
- **Misato = 帮我做**（执行层：拆任务、派活、追进度）

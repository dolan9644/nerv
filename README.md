...
| 中文触发 | 命中的任务流 | 核心能力 | 核心产物 | 主责角色 |
|:--|:--|:--|:--|:--|
| `帮我把这场直播复盘一下` / `整理下次优化点` | 直播复盘链（`live-replay-summary`） | `直播复盘` | 复盘总结、问题聚类、下一场优化清单 | `Misato` → `Eva-00` → `Eva-13` → `Rei` |
| `把这场直播的脚本做出来` / `给我一套能直接上播的话术` | 直播脚本链（`live-session-script`） | `直播脚本` | `offer_pack.json`、`script.md`、`selling_points.md`、`cta.md` | `Misato` → `Eva-00` → `Eva-13` → `Rei` |
| `给微博/小红书/抖音各出一版内容` | 多平台内容链（`social-copy-studio`） | `多平台内容` | 平台分发文案、视频脚本、口播稿 | `Misato` → `Eva-00` → `Eva-13` → `Rei` |
| `把这批商品评价整理成洞察` / `做一版 SKU 卖点简报` | 商品评价洞察链（`product-review-insight`） | `商品洞察` | 卖点洞察、痛点聚类、SKU 简报 | `Misato` → `Eva-00` → `Eva-13` |
| `把这段会议纪要转成任务清单` / `把周会内容拆成 owner 和 deadline` | 会议转任务链（`meeting-to-task`） | `会议转任务` | 任务清单、owner、deadline、状态摘要 | `Misato` → `Shinji` → `Eva-00`/`Eva-13` |
| `给我做一版今天的财讯简报` / `看一下这只股票最近有什么值得关注` | 财讯简报链（`finance-brief`） | `财讯简报` | 财讯简报、风险卡片、观察重点 | `Misato` → `Eva-02` → `Eva-00` → `Eva-13` |
| `我想看今天的社媒热点` / `先给我一版内容选题` | 社媒选题链（`social-topic-daily`） | `社媒选题` | 热点简报、选题推荐、标题建议 | `Eva-02` → `Eva-00` → `Eva-13` → `Misato` |
| `把这个视频整理成知识稿` / `给我一版视频总结` | 各平台视频链总结（`video-to-knowledge-base`） | `视频入库` | 转录文本、结构化总结、知识稿 | `Misato` → `Mari` / `Eva-00` / `Eva-13` |
| `按这个人物风格出一组图` / `给我一套可用的人物图` | 文生图（`digital-human-image-shoot`） | `人物出图` | 图片交付、镜头图组、参考图包 | `Misato` → `Eva-series` |
...
对所有重任务，NERV 会自动拆成多步任务流，保证输入、执行、收口和最终交付彼此分开，不再把复杂度硬塞进一个会话。
...
### 2. 各平台视频链总结：`video-to-knowledge-base`
...
### 3. 文生图：`digital-human-image-shoot`
...
1. **结果不是一次性试玩图**
   - 而是能继续复用的人物素材
2. **适合持续生产**
   - 同一人物、不同主题、不同镜头都能继续扩展

### 4. 会议转任务链：`meeting-to-task`
...
NERV 会继续保持“借鉴开源工程方法，但不把它们直接变成运行时依赖”的原则。
